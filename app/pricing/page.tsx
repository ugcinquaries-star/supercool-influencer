...
export default function PricingPage() {
  ...
  return (
    <div style={{ background: '#0A0608', minHeight: '100vh', color: 'white', fontFamily: 'sans-serif', padding: '80px 48px', textAlign: 'center' }}>
      <div style={{ fontSize: '12px', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#D4AF87', fontWeight: 600, marginBottom: '16px' }}>Pricing</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px', maxWidth: '1000px', margin: '0 auto 48px' }}>
        {plans.map(plan => (
          <div key={plan.id} style={{ background: plan.popular ? 'rgba(158,24,43,0.1)' : 'rgba(255,255,255,0.03)', border: `1px solid ${plan.popular ? 'rgba(158,24,43,0.5)' : 'rgba(255,255,255,0.08)'}`, borderRadius: '20px', padding: '32px 24px', position: 'relative', textAlign: 'left' }}>
            {plan.popular && <div style={{ position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)', background: 'linear-gradient(135deg, #9E182B, #D4AF87)', color: 'white', fontSize: '11px', fontWeight: 700, padding: '4px 16px', borderRadius: '100px', whiteSpace: 'nowrap' }}>MOST POPULAR</div>}
            ...
            <button
              ...
              style={{ width: '100%', padding: '14px', borderRadius: '10px', border: 'none', background: plan.popular ? 'linear-gradient(135deg, #9E182B, #D4AF87)' : 'rgba(255,255,255,0.08)', color: 'white', fontSize: '14px', fontWeight: 700, cursor: 'pointer' }}>
              Get {plan.name} →
            </button>
          </div>
        ))}
